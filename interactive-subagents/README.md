...
- **Parent task preview.** A `subagent` tool call uses Pi's native tool typography: bold lowercase `subagent`, the invocation name as the primary argument, and the unbracketed agent profile as secondary metadata, for example `subagent Auth flow · worker`. A blank line separates the header from one dimmed task-preview line. When task detail is hidden, the header shows the configured tool-expansion key (**Ctrl+O** by default); expanding restores the full task and logical line structure. Normal terminal text rendering displays tabs as spaces and treats CR/CRLF as line breaks. This is display-only and does not add another message to model context.
- **Delivered result preview.** A completed, failed, or user-stopped `subagent_result` uses the same identity grammar with muted status metadata, for example `subagent Auth flow · worker · completed in 5s`, followed by a blank line and up to two dimmed response-preview lines. It has no leading status icon. The self-rendered `Box(1, 1)` uses Pi's tool-success background for completions and tool-error background for failures and stops. Press the configured tool-expansion key (**Ctrl+O** by default) to show the complete Markdown response, context and cost, worktree outcome, session path, and resume guidance; collapse it to restore the preview. This changes only the interactive display - the parent model, session file, JSON mode, and print mode retain the complete result. Help requests stay fully visible and actionable rather than collapsing.
