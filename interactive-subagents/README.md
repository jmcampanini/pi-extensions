...
- **Live widget.** While children run, one line per sub-agent appears above the parent's editor: `[agent-type]  name` with a right-anchored elapsed clock — a row existing means it's running or just finished with its result message still queued (`delivering`, frozen clock). Names that repeat the agent type (`Scout: Auth` next to `[scout]`) are de-duplicated for display. The right edge carries the live status segment (see [Liveness](#liveness)); the status detail gives way before the name when the terminal narrows.
