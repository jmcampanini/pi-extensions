...
- **Parent task preview.** A `subagent` tool call uses Pi's native tool typography: bold lowercase `subagent start`, the invocation name as the primary argument, and the unbracketed agent profile as secondary metadata, for example `subagent start · worker · Auth flow`. A blank line separates the header from one dimmed task-preview line. When task detail is hidden, the header shows the configured tool-expansion key (**Ctrl+O** by default); expanding restores the full task and logical line structure. Normal terminal text rendering displays tabs as spaces and treats CR/CRLF as line breaks. This is display-only and does not add another message to model context.
- **Parent resume preview.** A `subagent_resume` tool call follows the same grammar, for example `subagent resume · worker · Auth flow`, followed by a blank line and one dimmed follow-up preview. The original name and agent are resolved from the child launch metadata unless the call overrides the display name. Expansion restores the complete follow-up message; a resume without one displays `No follow-up message.` without an expansion hint. The immediate successful launch acknowledgment remains model-visible but hidden from the human transcript, while launch errors remain visible.
- **Delivered result preview.** A completed, failed, or user-stopped `subagent_result` uses the same identity grammar with muted status metadata, for example `subagent result · worker · Auth flow · completed in 5s`, followed by a blank line and up to two dimmed response-preview lines. It has no leading status icon. The self-rendered `Box(1, 1)` uses Pi's tool-success background for completions and tool-error background for failures and stops. Press the configured tool-expansion key (**Ctrl+O** by default) to show the complete Markdown response, context and cost, worktree outcome, session path, and resume guidance; collapse it to restore the preview. This changes only the interactive display - the parent model, session file, JSON mode, and print mode retain the complete result. Help requests stay fully visible and actionable rather than collapsing.
