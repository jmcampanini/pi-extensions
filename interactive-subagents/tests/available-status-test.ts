...
state.running.set(
	"active01",
	runningChild("active01", "Implement parser", {
		agent: "worker",
...
state.running.set(
	"waiting1",
	runningChild("waiting1", "Await approval", {
		agent: "reviewer",
		autoExit: false,
...
state.running.set(
	"stalled1",
	runningChild("stalled1", "Recover liveness", {
		agent: "scout",
...
describe("tool registration", () => {
	it("discovery and status register only their canonical tool names and labels", () => {
		assert.deepStrictEqual(
			...
			["subagent_available", "subagent_status"],
...
describe("subagent_available", () => {
	...
	it("result carries versioned presentation details for the sorted inventory", () => {
		assert.strictEqual(
			availableDetails.presentation.version,
			1,
			"available result carries versioned presentation details",
		);
		assert.deepStrictEqual(
			availableDetails.presentation.inventory.map((entry) => entry.name),
			["scout", "worker"],
			"available details preserve the sorted definition inventory",
		);
		assert.deepStrictEqual(
			availableDetails.presentation.dirs,
			{
		...
		assert.ok(
			!availableText.includes("external:") && !availableText.includes("new-only"),
			"pi agents carry no external capability markers",
		);
		assert.deepStrictEqual(
			availableDetails.presentation.models,
			{
	...
	it("available call title uses bold tool-title styling", () => {
		const availableCallOutput =
			availableTool.renderCall?.({}, markedTheme, renderContext(false)).render(100).join("\n") ?? "";
	...
	it("collapsed available card shows compact markers and headlines with semantic tokens", () => {
		...
		assert.ok(
			collapsedAvailablePlain.includes(
				"scout · project · forked · interactive · worktree - Fast reconnaissance.",
		...
		assert.ok(
			!collapsedAvailablePlain.includes("Maps relevant code paths"),
			"collapsed available card omits expanded details",
		);
		assert.ok(
			!collapsedAvailablePlain.includes("external:") && !collapsedAvailablePlain.includes("new-only"),
	...
	it("expanded available card uses the versioned inventory with semantic tokens", () => {
		...
		assert.ok(
			expandedAvailablePlain.includes("── Models · 2 ─") &&
		...
		assert.ok(
			!expandedAvailablePlain.includes("external:") && !expandedAvailablePlain.includes("new-only"),
	...
	it("external cards advertise the harness and new-only capability", () => {
		...
		assert.ok(
			collapsedExternalPlain.includes(
				"claude-code · external: claude-code · new-only - Direct Claude Code handoff.",
			),
		...
		assert.ok(themeCalls.includes("warning"), "expanded external overview warning-paints capability metadata");
	});

	it("expanded available card shows the full description when details are absent", () => {
		const fallbackDescriptionInventory = availableDetails.presentation.inventory.map((agent) =>
			agent.name === "worker"
				? {
						...agent,
						description: "General implementation. Use this second sentence for routing.",
						details: undefined,
					}
				: agent,
		);
		const fallbackDescriptionCard = availableRenderer(
			{
				...availableResult,
				details: {
					presentation: { ...availableDetails.presentation, inventory: fallbackDescriptionInventory },
				},
			},
			{ expanded: true, isPartial: false },
			markedTheme,
		...
		assert.ok(
			stripVTControlCharacters(fallbackDescriptionCard.render(120).join("\n")).includes(
				"Use this second sentence for routing.",
			),
		);
	});
	...
	it("expanded available card is display-width safe and strips hostile sequences", () => {
		...
		for (const width of [1, 2, 8, 20, 40]) {
			assert.strictEqual(
				hostileAvailableCard.render(width).every((line) => visibleWidth(line) <= width),
				true,
...
describe("subagent_status", () => {
	it("empty status has one exact model-facing sentence with versioned details", () => {
		...
		assert.deepStrictEqual(
			emptyStatusDetails.presentation,
			{
				version: 1,
				entries: [],
			},
	...
	it("launch admission fills the last capacity slot then queues", () => {
		assert.strictEqual(startingAdmit.status, "run", "a fourth unresolved launch claims the last capacity slot");
		assert.strictEqual(queuedAdmit.status, "queued", "the next unresolved launch enters the capacity queue");
	});
	...
	it("active status exposes structured telemetry with current-run guidance", () => {
		...
		assert.ok(
			Boolean(
				activeEntry?.description.includes("running bash for 10s") &&
	...
	it("stalled status preserves last-known telemetry with honest labels", () => {
		...
		assert.ok(
			Boolean(
				stalledEntry?.description.includes("last reported context 54k/128k tokens") &&
	...
	it("delivering, stalled, starting, and queued guidance steer without false conclusions", () => {
		...
		assert.ok(
			Boolean(
				deliveringEntry?.state === "delivering" &&
				deliveringEntry.description.includes("stopped after 1m 35s") &&
				deliveringEntry.description.includes(
					"its stopped notice is queued and arrives on its own - end your turn to receive it",
				),
			),
		...
		assert.ok(
			Boolean(
				startingEntry?.description.includes("subagent_cancel") &&
		...
		assert.strictEqual(queuedEntry?.queuePosition, 1, "queued status carries its machine-readable queue position");
	});

	it("model-facing status format remains byte-for-byte unchanged", () => {
		assert.strictEqual(
			status.formatStatusModelText([
				{
					id: "agent 01",
					agent: "worker",
					name: 'Fix "parser"',
	...
	it("flat status emits ID-first rows and ends with the standing reminder", () => {
		...
		assert.ok(
			statusLines.every((line) => line.startsWith("• id ")) &&
				!["Summary:", "Total:", "Running subagents:", "Queued subagents:", "Unresolved subagents:"].some(
					(heading) => statusText.includes(heading),
				),
	...
	it("registered status execution preserves the attention order and unchanged model format", () => {
		assert.deepStrictEqual(
			...
			[1, ["delivery1", "stalled1", "waiting1", "starting1", "active01", "queued01"]],
			"registered status execution preserves the attention order in versioned details",
		);
		assert.strictEqual(
			liveStatusText,
			status.formatStatusModelText(liveStatusDetails.presentation.entries),
	...
	it("the status call keeps the configured expansion hint beside its bold title", () => {
		themeCalls.length = 0;
		const statusCallOutput =
			statusTool.renderCall?.({}, markedTheme, renderContext(false)).render(100).join("\n") ?? "";
		assert.ok(
		...
		const expandedStatusCallOutput =
			statusTool.renderCall?.({}, markedTheme, renderContext(true)).render(100).join("\n") ?? "";
		assert.strictEqual(
			stripVTControlCharacters(expandedStatusCallOutput).trimEnd(),
			"subagent status",
			"expanded status call removes the expansion hint",
		);
		assert.strictEqual(
			readFileSync(new URL("../tool-status.ts", import.meta.url), "utf8").includes(
	...
	it("expanded status is display-width safe and sanitizes hostile data", () => {
		...
		for (const width of [1, 2, 8, 20, 40]) {
			assert.strictEqual(
				hostileStatus.render(width).every((line) => visibleWidth(line) <= width),
				true,
