...
eq("rule 1: 60s-old problem, no snapshot ever accepted → stalled",
	computeStatus({ nowMs: T0 + STALL_AFTER_MS, watchdogStartMs: T0, expectsRun: true,
		problemSinceMs: T0 }),
	"stalled");
eq("rule 1 boundary: problem at 59_999ms is NOT stalled (still starting)",
	computeStatus({ nowMs: T0 + STALL_AFTER_MS - 1, watchdogStartMs: T0, expectsRun: true,
		problemSinceMs: T0 }),
	"starting");
...
eq("rule 1 overrides an old accepted ACTIVE snapshot",
	computeStatus({ nowMs: T0 + STALL_AFTER_MS, watchdogStartMs: T0 - 3_600_000, expectsRun: true,
...
eq("rule 2: no snapshot, no problem → starting",
	computeStatus({ nowMs: T0, watchdogStartMs: T0, expectsRun: true }),
	"starting");
eq("rule 2: a problem still debouncing (under 60s) reads starting, not stalled",
	computeStatus({ nowMs: T0 + 30_000, watchdogStartMs: T0, expectsRun: true,
...
eq("rule 2: no snapshot at 10h age with no problem → still starting",
	computeStatus({ nowMs: T0 + 36_000_000, watchdogStartMs: T0, expectsRun: true }),
	"starting");
...
eq("rule 4: expectsRun false with zero runs waits immediately (human handoff)",
	computeStatus({ nowMs: T0, watchdogStartMs: T0, expectsRun: false,
		snapshot: snap({}) }),
	"waiting");
eq("rule 4: expectsRun false with zero runs still waiting past the watchdog",
	computeStatus({ nowMs: T0 + STALL_AFTER_MS * 10, watchdogStartMs: T0, expectsRun: false,
		snapshot: snap({}) }),
	"waiting");
...
eq("rule 5: implant alive, prompted run not begun, under 60s → starting",
	computeStatus({ nowMs: T0 + 5_000, watchdogStartMs: T0, expectsRun: true,
		snapshot: snap({}) }),
	"starting");
...
{
	const stalledInput = { nowMs: T0 + STALL_AFTER_MS, watchdogStartMs: T0, expectsRun: true,
		snapshot: snap({ inRun: true }), problemSinceMs: T0 };
...
// applying the shift inline so the test needs no runtime activity.ts import.
{
	...
	eq("suspend: 5-minute gap re-anchors the watchdog → still starting",
		computeStatus({ nowMs, watchdogStartMs, expectsRun: true, snapshot: snap({}) }),
		"starting");
	eq("suspend control: WITHOUT the shift the same tick would misread stalled",
		computeStatus({ nowMs, watchdogStartMs: T0, expectsRun: true, snapshot: snap({}) }),
		"stalled");
}
