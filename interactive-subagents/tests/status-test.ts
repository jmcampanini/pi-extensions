...
describe("computeStatus", () => {
	...
	it("rule 4: expectsRun false with zero runs waits immediately (human handoff)", () => {
		assert.strictEqual(
			computeStatus({ nowMs: T0, watchdogStartMs: T0, expectsRun: false, everSawRun: false, snapshot: snap({}) }),
			"waiting",
		);
	});
	...
	it("stalled is computed, never latched", () => {
		...
		assert.strictEqual(
			computeStatus({
				...stalledInput,
				snapshot: snap({ inRun: true, updatedAt: 2_000, sequence: 7 }),
