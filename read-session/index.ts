...
export default function (pi: ExtensionAPI) {
	...
	const openReader = async (ctx: ExtensionContext): Promise<void> => {
		if (exporting) {
	...
	pi.registerCommand("read-session", {
		...
		handler: (_args, ctx) => openReader(ctx),
	});
	if (config.openShortcut !== null) {
		pi.registerShortcut(config.openShortcut, {
			description: "Open the current session in the browser reader",
			handler: openReader,
		});
	}
}
