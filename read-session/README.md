...
Run `/read-session` in Pi to open the current conversation in your browser. The page shows the newest messages first, with your prompts in full-width outlined cards in the center thread and a sticky copy on the right for context. Read and copy text in the browser, then reply in Pi.
...
Messages and tool calls appear newest first. Paragraphs, lists, code, and headings keep their original order inside each message. On a large screen, the outline sits on the left and the conversation in the center. Your prompt appears below its responses to preserve newest-first order, with a sticky copy on the right. Both copies include the full Markdown and have distinct heading and copy targets. A narrower screen moves the outline into a floating menu. Phones and printed pages show only the inline prompt.

Inline prompt cards have no role label or header row. Agent messages have no role label, dot, or header row. Their copy buttons appear in the corner on hover or keyboard focus and remain visible on touchscreens. Interrupted and aborted messages still show their status.
...
| `components/message.html` | Context labels, interruption statuses, copy controls, and content placement |
...
After editing a template or stylesheet, run `node read-session/preview.ts` again and refresh the open page. The command replaces `.sandbox/read-session-preview/components.html` and `index.html`. The samples include short prompts, long Markdown with code, skills, steering, and interrupted responses. The gallery's layout lives in `gallery.html` and `gallery.css`; it does not change the page generated by `/read-session`.
