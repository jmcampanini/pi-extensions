...
Subagent traffic gets structured treatment (via the format contract exported by the interactive-subagents extension): spawn/resume rows show `name=… agent=…` metadata while the preview and detail show the full task prompt. Result rows keep name, agent, and status first, followed by the remaining envelope fields in canonical order. Rendered result previews and details show the delivered response first, unwrapped from its `<result>` markers, then an aligned metadata table containing the action, session, and worktree tail. Bounded previews keep the response and clip trailing table rows first; raw mode, copy, and open keep the stored envelope unchanged.
