...
describe("explorer frame end-to-end", () => {
	...
	it("a real branched session fixture drives the complete frame", (t) => {
		...
		for (const width of [120, 100]) {
			...
			assert.ok(lines[0]?.startsWith("┌ fuzzy ") === true && lines[0]?.endsWith(" 13/13 ┐") === true,
			...
			assert.ok(lines.some((line) => line.includes("read") && line.includes("path=docs/guide.md")),
		...
		for (const width of [120, 100]) {
			...
			assert.ok(lines[1]?.startsWith("│ path=src/config.ts") === true && lines[2]?.startsWith("│ offset=12") === true,
				`detail@${width}: rendered read leads with its arguments`);
			assert.ok(lines.some((line) => line.includes("STORED_RESULT_ONLY_NEEDLE")) && !lines.some((line) => line.includes("read {")),
			...
			assert.ok(lines.at(-1)?.includes("m raw") === true,
				`detail@${width}: highlighted-by-default code read offers the raw toggle`);
		}
		component.handleInput("m");
		const rawRead = component.render(120);
		assert.ok(rawRead[1]?.startsWith("│ read {") === true && rawRead.at(-1)?.includes("m code") === true,
		...
		assert.deepStrictEqual(state.results.map((result) => result.block.toolCallId), ["call-read-md"],
			"query narrows to the markdown read");
		component.handleInput("\r");
		const markdownRead = component.render(120);
		assert.ok(markdownRead[1]?.startsWith("│ path=docs/guide.md") === true
