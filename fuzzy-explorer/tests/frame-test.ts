...
function frameChecks(width: number, lines: string[], label: string): void {
	...
	assert.ok(
		lines[0]?.startsWith("┌") === true &&
			lines[0]?.endsWith("┐") === true &&
			lines.at(-1)?.startsWith("└") === true &&
	...
	assert.ok(
		lines
			.slice(1, -1)
			.every(
				(line) => (line.startsWith("│") && line.endsWith("│")) || (line.startsWith("├") && line.endsWith("┤")),
			),
...
describe("explorer frame end-to-end", () => {
	...
	it("fixture extracts the expected active-branch blocks", () => {
		assert.strictEqual(blocks.length, 13);
	});
	...
	it("a real branched session fixture drives the complete frame", (t) => {
		...
		for (const width of [120, 100]) {
			...
			assert.ok(
				lines[0]?.startsWith("┌ fuzzy ") === true && lines[0]?.endsWith(" 13/13 ┐") === true,
				`list@${width}: top border embeds title and totals`,
			);
			assert.ok(
				lines[1]?.startsWith("│ › / to filter") === true,
			...
			assert.ok(
				lines.some((line) => line.includes("read") && line.includes("path=docs/guide.md")),
				`list@${width}: tool rows show their argument details`,
			);
			assert.ok(
				lines.some((line) => line.startsWith(`├ User · ${newestTime} `)),
				`list@${width}: preview rule carries the block identity`,
			);
			assert.ok(
				lines.some((line) => line.startsWith("│ latest active user block")),
				`list@${width}: preview shows the selected body`,
			);
			assert.ok(
				lines.at(-1)?.startsWith("└ l/enter detail · / filter · q/esc quit") === true,
		...
		assert.deepStrictEqual(
			state.results.map((result) => result.block.toolCallId),
			["call-read"],
			"fuzzy query narrows to the read invocation",
		);
		const filtered = component.render(120);
		frameChecks(120, filtered, "filter@120");
		assert.ok(filtered[0]?.includes(" 1/13 ┐") === true, "filter@120: counts show narrowed matches");
		assert.ok(
		...
		assert.ok(
			excerptFrame.some(
				(line) =>
					line.includes("▸ read") && line.includes("STORED_RESULT_ONLY_NEEDLE") && !line.includes("path="),
			),
			"body-only rows swap in the matching excerpt",
		);

		// Detail frame: identity in the border, a code read opens highlighted with its arguments first.
		...
		for (const width of [120, 100]) {
			...
			assert.ok(
				lines[1]?.startsWith("│ path=src/config.ts") === true && lines[2]?.startsWith("│ offset=12") === true,
				`detail@${width}: rendered read leads with its arguments`,
			);
			assert.ok(
				lines.some((line) => line.includes("STORED_RESULT_ONLY_NEEDLE")) &&
					!lines.some((line) => line.includes("read {")),
				`detail@${width}: rendered content shows the stored result without the invocation header`,
			);
			assert.ok(
				lines.at(-1)?.startsWith("└ j/k scroll") === true,
				`detail@${width}: hints render in the bottom border`,
			);
			assert.ok(
				lines.at(-1)?.includes("o open src/config.ts:12") === true,
				`detail@${width}: hints include the smart-open target`,
			);
			assert.ok(
				lines.at(-1)?.includes("m raw") === true,
				`detail@${width}: highlighted-by-default code read offers the raw toggle`,
			);
		}
		component.handleInput("m");
		const rawRead = component.render(120);
		assert.ok(
			rawRead[1]?.startsWith("│ read {") === true && rawRead.at(-1)?.includes("m code") === true,
			"m reveals the canonical invocation text and offers the code toggle",
		);

		// Markdown read from the real session file renders its markup.

		component.handleInput("\x1b");
		component.handleInput("/");
		component.handleInput("\x15");
		component.handleInput("guide");
		assert.deepStrictEqual(
			state.results.map((result) => result.block.toolCallId),
			["call-read-md"],
			"query narrows to the markdown read",
		);
		component.handleInput("\r");
		const markdownRead = component.render(120);
		assert.ok(
			markdownRead[1]?.startsWith("│ path=docs/guide.md") === true &&
				markdownRead.some((line) => line.startsWith("│ Fixture guide")) &&
				!markdownRead.some((line) => line.includes("# Fixture guide")) &&
				markdownRead.at(-1)?.includes("m raw") === true,
			"markdown read renders headings after the path line",
		);
		...
		assert.ok(
			roundTrip.some((line) => line.includes("▸ read")),
			"round trip keeps the filtered selection",
		);
	});
});
