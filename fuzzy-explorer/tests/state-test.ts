...
describe("ExplorerState", () => {
	it("navigates, filters, syncs, and transitions modes over one live session", () => {
		...
		assert.strictEqual(state.selected?.block.id, "entry-7:user", "cursor starts at the newest chronological block");
		...
		assert.deepStrictEqual(
			[state.results.length, state.selected],
			[0, undefined],
		...
		assert.deepStrictEqual(
			[state.selected?.block.id, state.listViewport],
			["entry-5:user", 3],
		...
		state.setQuery("");
		assert.deepStrictEqual(
			[state.selected?.block.id, state.selectedIndex],
			["entry-8:user", 8],
			"an empty query re-anchors on the newest block",
		);
		assert.deepStrictEqual(
			[state.results[0]?.block.id, state.results.at(-1)?.block.id],
		...
		assert.deepStrictEqual(
			[state.escape(), state.mode, state.query],
			[false, "list", "body"],
		...
		state.jumpDetail(1);
		assert.deepStrictEqual(
			[state.selected?.block.id, state.mode],
			["entry-7:user", "detail"],
		...
		assert.deepStrictEqual(
			[state.escape(), state.mode, state.selected?.block.id],
			[false, "list", "entry-7:user"],
			"Escape returns from detail with selection synced",
		);

		state.enterDetail();
		assert.strictEqual(state.detailRenderedOverride, undefined, "detail opens with the policy-default view");
		state.setDetailPageSize(4);
		state.scrollDetail(2, 20);
		state.toggleDetailRendered(true);
		assert.deepStrictEqual(
			[state.detailRenderedOverride, state.detailOffset],
			[false, 0],
