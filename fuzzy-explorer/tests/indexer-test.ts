...
const result = {
	...
	message: {
		...
		toolCallId: "call-1",
		toolName: "read",
		content: [{ type: "text", text: "complete stored result" }],
		isError: false,
		timestamp: 3,
	},
} as SessionEntry;
const alternate = {
	...base("04", "01"),
	type: "message",
	message: { role: "user", content: "alternate active branch", timestamp: 4 },
} as SessionEntry;
...
describe("BranchBlockIndex", () => {
	it("incrementally reuses, merges, and rebuilds blocks across branch updates", () => {
		...
		assert.deepStrictEqual(
			rebuilt.map((block) => block.body),
			["start", "alternate active branch"],
		...
		assert.deepStrictEqual(
			[labeled.length, labeled[1]?.label],
			[2, "checkpoint"],
