...
export function buildFixtureSession(directory: string, additionalMessages = 0): FixtureSession {
	...
	add({
		...base("00000001", null, 1),
		type: "message",
		message: {
			role: "user",
			content: "active root asks about src/config.ts",
			timestamp: 1,
		},
	});
	...
	add({
		...base("a0000001", "00000001", 2),
		type: "message",
		message: {
			role: "user",
			content: "ABANDONED_SECRET_NEVER_INDEX",
			timestamp: 2,
		},
	});
	add({
		...base("a0000002", "a0000001", 3),
		type: "message",
		message: {
			role: "assistant",
			content: [{ type: "text", text: "abandoned assistant text" }],
	...
	add({
		...base("00000002", "00000001", 4),
		type: "branch_summary",
		fromId: "a0000002",
		summary: "Returned from an abandoned experiment.",
	});
	add({
		...base("00000003", "00000002", 5),
		type: "message",
		message: {
			role: "assistant",
			content: [
	...
	add({
		...base("00000007", "00000006", 9),
		type: "message",
		message: {
			role: "toolResult",
			toolCallId: "call-truncated",
			toolName: "grep",
			content: [{ type: "text", text: "src/large.ts:42:stored truncated needle" }],
			isError: false,
			timestamp: 9,
			details: {
				fullOutputPath,
				truncation: {
					content: "src/large.ts:42:stored truncated needle",
					truncated: true,
					truncatedBy: "lines",
					totalLines: 5000,
					totalBytes: 100000,
	...
	add({
		...base("00000008", "00000007", 10),
		type: "message",
		message: {
			role: "assistant",
			content: [{ type: "toolCall", id: "call-gone", name: "read", arguments: { path: "gone.log" } }],
			api: "openai-completions",
			provider: "openai",
			model: "fixture",
			usage,
	...
	add({
		...base("00000009", "00000008", 11),
		type: "message",
		message: {
			role: "toolResult",
			toolCallId: "call-gone",
			toolName: "read",
			content: [{ type: "text", text: "last stored lines only" }],
			isError: true,
			timestamp: 11,
			details: {
				fullOutputPath: missingFullOutputPath,
				truncation: {
					content: "last stored lines only",
					truncated: true,
					truncatedBy: "bytes",
					totalLines: 9000,
					totalBytes: 900000,
	...
	add({
		...base("0000000a", "00000009", 12),
		type: "message",
		message: {
			role: "toolResult",
			toolCallId: "orphan-call",
			toolName: "mystery",
			content: [{ type: "text", text: "orphan result remains visible" }],
			isError: false,
			timestamp: 12,
		},
	});
	add({
		...base("0000000b", "0000000a", 13),
		type: "message",
		message: {
			role: "bashExecution",
			command: "printf bash-fixture",
			output: "bash-fixture",
			exitCode: 0,
			cancelled: false,
			truncated: false,
	...
	add({
		...base("0000000c", "0000000b", 14),
		type: "message",
		message: {
			role: "custom",
			customType: "fixture-card",
	...
	add({
		...base("0000000d", "0000000c", 15),
		type: "custom_message",
		customType: "fixture-entry",
		content: "visible custom entry message",
		display: true,
	});
	add({
		...base("0000000e", "0000000d", 16),
		type: "custom_message",
		customType: "fixture-hidden",
		content: "HIDDEN_CUSTOM_NEVER_INDEX",
		display: false,
	});
	add({
		...base("0000000f", "0000000e", 17),
		type: "compaction",
		summary: "Compaction summary fixture text.",
		firstKeptEntryId: "00000001",
		tokensBefore: 12345,
	});
	add({
		...base("00000011", "0000000f", 18),
		type: "message",
		message: {
			role: "assistant",
	...
	add({
		...base("00000012", "00000011", 19),
		type: "message",
		message: {
			role: "toolResult",
			toolCallId: "call-read-md",
			toolName: "read",
			content: [{ type: "text", text: FIXTURE_GUIDE_MARKDOWN }],
			details: {},
			isError: false,
			timestamp: 19,
		},
	...
	add({
		...base("00000010", "00000012", 20),
		type: "message",
		message: {
			role: "user",
			content: [
				{ type: "text", text: "latest active user block" },
				{
					type: "image",
					data: "BASE64_IMAGE_NEVER_INDEX",
					mimeType: "image/png",
				},
			],
			timestamp: 20,
	...
	for (let index = 0; index < additionalMessages; index++) {
		...
		add({
			...
			message: {
				role: "user",
				content: `large-session-row-${String(index).padStart(5, "0")}`,
